 ...
 * mode this build has no entry for, and any app on the device can send an arbitrary one. These
 * throw instead of indexing, which the dispatch helpers turn into an `onError` naming the value.
 */
internal fun requireShareMode(ordinal: Int): ShareMode =
    ShareMode.fromOrdinal(ordinal)
        ?: throw IllegalArgumentException(
            "Unknown ShareMode ordinal $ordinal — this build knows ${ShareMode.entries}",
        )

internal fun requireShareReceiver(kind: Int, value: String?): ShareReceiver =
