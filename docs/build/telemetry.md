...
and the `WWW-Authenticate` value truncated to 60 characters (`SolidHttpClient.kt:761`, `:771`);
...
plus message (`SolidHttpClient.kt:657`). The one `setKey` call site is the IPC host stamping
...
- **Server strings are truncated.** The `WWW-Authenticate` header is cut to 60 characters before
  it enters a breadcrumb (`SolidHttpClient.kt:763`). OAuth `errorDescription` strings go to
