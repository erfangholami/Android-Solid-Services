 ...
 *
 * Without this, `Solid.getResourceClient(...)` binds to the installed host app — which is either
 * absent (every call fails at once) or real (the tests would write to a live pod). Redirecting the
 * host package is enough because the fakes answer to the production intent actions.
 *
...
class FakeSdk : TestWatcher() {
    ...
    override fun starting(description: Description) {
        HostResolver.overrideHostPackage = context.packageName
