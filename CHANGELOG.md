...
- **A host can see the apps it granted.** Android 11 package visibility hid them. `client` declared
  `<queries>` for the host, and nothing pointed back, so `PackageManager.getApplicationInfo` was
  refused: the host could name a granted app by its package alone, drew no icon, and reported a live
  grant as uninstalled. Visibility earned from an interaction, such as the consent screen, is not
  kept, so updating or reinstalling either app brought the fault back. `client` now exports an inert
...
- Apps built against 0.8.0 keep working, but stay unnamed in the host's list. Rebuilding is the fix.
- Read the label and icon whenever you draw them, not when the grant is made, so an app that
  rebrands in an update shows its new name at once.
