...
**Fix:** Launch the `AuthorizeWithSolid` contract from your Activity — the picker opens in your own foreground, the chosen WebID comes back as an activity result, and no permission is involved. See [Getting Started](https://androidsolidservices.erfangholami.com/0.7/start/quickstart/index.md).
...
`probeAccess` wraps this in a typed result if you would rather not read header sets — see [Access control](https://androidsolidservices.erfangholami.com/0.7/build/access-control/index.md).
